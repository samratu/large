...
DAEMON_OPTS="--user sslh --listen 0.0.0.0:443 --ssl 127.0.0.1:500 --ssh 127.0.0.1:300 --openvpn 127.0.0.1:1194 --http 127.0.0.1:8080 --pidfile /var/run/sslh/sslh.pid -n"
...
cert = /etc/xray/xray.cer /etc/xray/xray.key
client = no
...
[Service]
ExecStart=/usr/local/wisnucs/stunnel /etc/stunnel5/stunnel5.conf
Type=forking
