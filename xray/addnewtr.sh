...
trojanhdua="trojan://${uuid}@${domain}:$thdua?sni=${domain}&type=http&security=tls&path=%2fgandring#${user}"
trojangrpc="trojan://${uuid}@${domain}:$tgrpc?mode=gun&security=tls&type=grpc&serviceName=%2fgandring&sni=${domain}#${user}"
...
trojanhttp="trojan://${uuid}@${domain}:$thttp?sni=${domain}&type=tcp&security=tls&&host=$domain&path=/gandring&headerType=http#${user}"
...
echo -e "ServiceName/path :/gandring"
